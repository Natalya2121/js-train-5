...
function addTenToEach(array) {
  ...
  // Виводимо результат
  let resArray = Array(array.length);
  for (const idx in array) {
    resArray[idx] = array[idx] + 10;
  }
...
function replaceNumbers(arr) {
  ...
  let resArr = [];
  for (const idx in arr) {
    if (arr[idx] > 10) resArr[idx] = "Greater than 10";
    else resArr[idx] = "Less than or equal to 10";
  }
  return resArr;
}
